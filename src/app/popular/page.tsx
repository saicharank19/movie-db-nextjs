...
function Popular() {
    ...
    const getPopularMovies = async () => {
      try {
        ...
        const response = await axios.get(
          `https://api.themoviedb.org/3/movie/popular?api_key=500a2293d6403e9c3caedb4591ae7624&page=${currentPage}`
        );
        if (response) {
  ...
  return loading ? (
    ...
    <div>
      <h1 className="pl-2 text-2xl font-bold">Popular Movies</h1>
      <div className="flex flex-wrap w-full justify-center">
        {popularList.map((eachMovie: Movie, index: number) => {
          return (
            <div
              key={index}
